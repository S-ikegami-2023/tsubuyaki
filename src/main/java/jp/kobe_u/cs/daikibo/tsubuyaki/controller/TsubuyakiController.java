...
public class TsubuyakiController {
    ...
    String showTsubuyakiList(Model model) {
        ...
        model.addAttribute("tsubuyakiSearch", new TsubuyakiSearch());  //空フォームをセット
    ...
    //キーワードを含むつぶやきを検索結果画面に表示
    @GetMapping("/read/Search")
    String showTsubuyakiSearchList(@ModelAttribute("tsubuyakiSearch") TsubuyakiSearch searched, Model model){
        List<Tsubuyaki> seachedList = ts.getSearchedTsubuyaki(searched.getSearch());
        model.addAttribute("tsubuyakiSearchList", seachedList);   //モデル属性にリストをセット
        model.addAttribute("tsubuyakiSearch", new TsubuyakiSearch());  //空フォームをセット
        return "tsubuyaki_searched_list"; //検索画面を返す
